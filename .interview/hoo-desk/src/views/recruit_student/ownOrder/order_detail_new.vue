<template>
  <div v-loading='fullLoading' class="student-order-details">
    ...
    <!-- end -->
    <v-course-box @onSubmit='toEidtOrd' :payStatus='payStatus' v-if='Object.keys(orderInfo).length !==0' :info='orderInfo'></v-course-box>
    <div class='other-fee'>
      <div class='tips-bar'>3.添加其他收费(输入负数表示优惠金额，如"-100")</div>
    ...
    <div class='pay-list-wrap'>
        ...
        <el-row type='flex'>
          ...
          <el-col :span='4'>
            <el-input type="text"
                      ...
                      :placeholder="addOrderPay.payment!='钱包余额'?'请输入收款金额':'最大可用'+balance+'元'"></el-input>
          </el-col>
          <el-col :span='3'>
            <el-input type="text"
      ...
      <el-table class="pay-list"
        ...
        <el-table-column label='付款方式'>
          <template slot-scope="scope">
            {{scope.row.payment}}
            <span v-if='scope.row.payment != "钱包余额"'>
              <el-button type='text'
        ...
        <el-table-column label='操作'>
          <template slot-scope="scope">
            <el-button v-if="scope.row.print_count>0&&(scope.row.apply_id == null || scope.row.apply_id == 0)&&scope.row.der_id == 0 && scope.row.is_del == 0"
                       type="primary"
                       @click="goApply(scope.row)">申诉</el-button>
            <el-button v-if="scope.row.print_count==0&&(scope.row.apply_id == null || scope.row.apply_id == 0)&&scope.row.is_del==0&&scope.row.der_id == 0"
                       type="primary"
    ...
    <!--修改支付方式-->
    <el-dialog :visible.sync="showChangePayType">
...
import courseBox from "@/views/recruit_student/ownOrder/order_details/course_box";
export default {
  data() {
    return {
      payMethodList: [],
  ...
  //重新申述 
   ...
   //匹配男女 
    ...
    getOrderDetail() {
      this.fullLoading = true;
      orderDetail(this.$route.query.order_id)
        .then(res => {
    ...
    newPrint(){
      ...
      let content = "";
        ...
        if (
          ...
          content +="<tr><td>课程名称:"+course.subject_name_print+"(共"+course.times+"次)\n</td></tr>";
          if(parseFloat(discount_price) != 0){
  ...
  computed: {
    ...
    payStatus() {
      if (this.orderInfo && this.orderInfo.pay_list.length == 0) {
        return false;
      } else {
        return true;
      }
