...
export default {
  ...
  methods: {
    ...
    * @param  Boolean     {data}
    ...
    /**
    * getReportList 获取报告列表{{ scope.row.open_count }}/{{ scope.row.student_count }}
    * @param  Boolean     {name}
    * @param  Boolean     {value}
    * @param  Boolean     {data}
...
.index-content >>> .index-wrap
  margin-bottom 0
.index-content >>> .search-bar
  margin-bottom 0
.index-content >>> .pub-table-wrap
  padding-top 0
.index-content >>> .pub-filter-box
  display none

.index-content >>> .dialog-wrap .el-dialog__header
  height 0
.index-content >>> .dialog-wrap .el-dialog__headerbtn
  top -40px
.index-content >>> .dialog-wrap .el-dialog__close
  font-size 25px
  color #fff
.index-content >>> .dialog-wrap .el-dialog__body
  position relative
  padding 0
</style>
