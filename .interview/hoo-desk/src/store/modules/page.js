  ...
  actions: {
    ...
    add ({ state, commit, dispatch }, { tag, params, query, fullPath }) {
      return new Promise(async resolve => {
        ...
        // console.log('%ctag','font-size:40px;color:pink',tag.name)
        let newTag = tag
    ...
    open ({ state, commit, dispatch }, { name, params, query, fullPath }) {
      console.log('%ctag%cpath', 'font-size:40px;color:pink', name, fullPath)
      return new Promise(async resolve => {
        // 已经打开的页面
    ...
    close ({ state, commit, dispatch }, { tagName }) {
      return new Promise(async resolve => {
        ...
        // 最后需要判断是否需要跳到首页
        if (isCurrent) {
          const { name = '', params = {}, query = {}, path = '' } = newPage
          let routerObj = {
            path,
            params,
            query
    ...
    closeAll ({ state, commit, dispatch }) {
      return new Promise(async resolve => {
        ...
        if (router.app.$route.name !== 'entry') {
          router.push({
            path: '/entry'
          })
        }
